<template>
  <div class="project-transaction">
    <div class="project-transaction__container">
      ...
      />
      <div v-if="showVoteBtn && projectData.contractAddress">
        <div class="wallet">
          <span>Contract:</span>
        ...
        <div v-if="false">
          <div>contractBalance: {{ showAmountUSD(contractBalance / 1) }}</div>
          <div>userBalance: {{ showAmountUSD(userBalance / 1) }}</div>
        </div>
      </div>
    ...
    >
      <div style="font-weight: 900; padding-bottom: 10px">Owner area</div>
      <div v-if="crowdFinish && contractData.returnStartTime / 1 > 0">
        <div>Payouts:</div>
        <div v-if="testMode" style="font-size: 10px">
          Token {{ tokenAddress }}
        </div>
        <div
          ...
          <div>
            <div>
              <small style="font-size: 10px"
                >Payment time: {{ convertSecondsToDate(item.time) }}</small
              >
            </div>
            <div>Amount: {{ showAmountUSD(item.amount) }}</div>
          </div>
          <div
            v-if="item.completed / 1 > 0"
            style="font-size: 12px; color: green"
          >
            Payed {{ convertSecondsToDate(item.completed) }}
          </div>
          <div v-if="item.completed == 0" style="flex-shrink: 0">
            <BaseButton
              v-if="contractData.getNow && contractData.getNow > item.time"
              color="blue"
              style="width: min-content"
              @click="makeReturnForAllUsers(item.amount / 1)"
            >
              Pay
            </BaseButton>
          </div>
        ...
        <div v-if="isOwner">
          <BaseInput v-model="nowValue" />
          <BaseButton color="blue" @click="setNow"> Set Now </BaseButton>
        </div>
      </div>
      <!--withdraw-->
      <div v-if="crowdFinish && contractData.returnStartTime / 1 === 0">
        <div v-if="!contractWait">
          <BaseInput v-model="withdrawTo" :placeholder="'Recipient address'" />
          <BaseButton
        ...
        >
          <div>
            <div>
              <small style="font-size: 10px">{{
                convertSecondsToDate(item.time)
              }}</small>
            </div>
            <div style="font-size: 12px">
              Amount: {{ showAmountUSD(item.amount) }}
            </div>
            <div style="font-size: 12px">Recipient: {{ item.recipient }}</div>
          </div>

          <BaseButton
            v-if="!item.approved"
            color="blue"
            @click="approveWithdraw(item.id)"
          >
            Approve withdraw
          </BaseButton>

          <div v-if="item.approved">Wait all approves</div>
        </div>
      </div>
    </div>
  </div>
